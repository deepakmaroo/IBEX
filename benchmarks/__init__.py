uris_label = ["pulsefile uri"]
uris = [
    # "imas:mdsplus?user=public;database=iter;pulse=135012;run=1;version=3",  # 245 slices
    # "imas:mdsplus?user=public;database=iter;pulse=135014;run=1;version=3",  # 984 slices
    # "imas:mdsplus?user=public;database=ITER;pulse=135009;run=5;version=3",  # 11808 slices
    # "imas:mdsplus?user=public;database=ITER;pulse=135010;run=5;version=3",  # 70340 slices
    "imas:mdsplus?user=public;database=ITER;pulse=135002;run=5;version=3",  # 127340 slices
    "imas:hdf5?user=public;database=ITER;pulse=135002;run=5;version=3",  # 127340 slices
]
