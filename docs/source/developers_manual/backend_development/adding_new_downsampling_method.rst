...
Class :py:class:`ibex.core.utils.DownsamplingMethods` is an ``Enum`` that defines the available downsampling strategies.
To make a new method available, a developer has to extend the enum by adding a new entry that includes the method's name,
...
``method_name(data: IDSNumericArray, n_out: int, *args, **kwargs)`` where ``data`` is an ``IDSNumericArray`` containing the input data, and ``n_out`` specifies the target number of output samples.
The function must return a valid index object suitable for slicing the data array. For example, returning :py:data:`slice(None, None, 2)` will effectively reduce the number of data points by half when applied as:
``data[method_name(...)]``
...
The primary entry point for downsampling logic is the :py:class:`ibex.core.utils.downsample_data` function.
